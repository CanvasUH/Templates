...
this.mmooc.api = function() {
    ...
    return {
         ...
         */
        listModulesForCourse: function(callback, cid)
        {
        ...
        getModulesForCurrentCourse: function(callback, error) {
            ...
            this.listModulesForCourse(callback, courseId);
        },
		...
		mmooc.api.getModulesForCurrentCourse(function(modules) {
			...
			for (var i = 0; i < noOfModules; i++) {
				...
				for (var j = 0; j < m.items.length; j++) {
					totalItems = totalItems + m.items.length;
					var item = m.items[j];
					var completed = false;
					...
					{
						html = html + '<a class="btn btn-warning" role="button" href="' + item.html_url + '">&nbsp;</a>';
					}                
				}
				html = html + "</div>";
			}
		...
		if(kuppModuleMenu.length)
		{
			mmooc.api.getCurrentModule(function(module) {
				...
				for (var i = 0; i < module.items.length; i++) {
					...
					if(item.isCurrent) { html += '<li class="active-kupp-menu-item">' + item.title + '</li>'; }
					else {html += '<li><a href="' + item.html_url + '">' + item.title + '</a></li>';}
				}
				html += '</ul>';
